...
- SQLite가 없어도 worker는 summary index로 queued/running 후보를 찾고, 선택 후보만 `job.json`을 읽습니다.
- artifact 목록은 짧은 TTL cache와 pagination을 사용합니다.
- artifact download는 file stream을 사용하고, CSV preview는 750KB/500행으로 제한합니다.
- bundle download는 파일을 읽기 전에 파일 수와 총 용량을 먼저 검사합니다.
- integrity hash는 stream 기반 SHA-256과 제한된 concurrency를 사용합니다.
- 외부 tool log와 event log는 전체 파일 read 대신 tail window만 읽습니다.
