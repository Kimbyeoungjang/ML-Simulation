...
| `GET /api/jobs/:id/artifacts` | artifact 목록 조회 | UI 파일 탭에서 사용 |
| `GET /api/jobs/:id/artifacts/:name` | artifact 다운로드 | path safety 검사 |
| `GET /api/jobs/:id/bundle` | job bundle zip 다운로드 | 제출/공유용 |
...
| `GET/PATCH /api/env` | `.env` 값 조회/수정 | 설정 탭 |
...
| `TILEFORGE_JOBS_DASHBOARD_ARTIFACTS` | job dashboard artifact eager loading 여부 |
