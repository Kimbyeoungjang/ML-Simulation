...
브라우저에서 기본값 기준 `http://127.0.0.1:3000`을 엽니다. 이 명령은 웹 서버와 worker를 동시에 실행하며, 웹 서버의 포트와 bind 주소는 `.env`의 `TILEFORGE_WEB_PORT`, `TILEFORGE_WEB_HOST`로 바꿀 수 있습니다.

## 3. 포트/API 주소 변경

`.env` 또는 UI의 설정 탭에서 다음 값을 수정합니다.

| 변수 | 의미 | 기본값 | 예시 |
|---|---|---|---|
| `TILEFORGE_WEB_PORT` | Next.js 웹 서버 포트 | `3000` | `4000` |
| `TILEFORGE_WEB_HOST` | 웹 서버 bind 주소 | `127.0.0.1` | `0.0.0.0` |
| `NEXT_PUBLIC_TILEFORGE_API_BASE_URL` | 브라우저가 호출할 API base URL | 빈 값 | `http://127.0.0.1:4000` |

일반적인 단일 서버 실행에서는 `NEXT_PUBLIC_TILEFORGE_API_BASE_URL`을 비워두는 것이 가장 안전합니다. 이 경우 모든 API 호출은 현재 접속한 서버의 `/api`로 들어갑니다. 포트, host, `NEXT_PUBLIC_` 값은 클라이언트 번들/서버 시작 시점에 읽히므로 저장 후 `npm run dev` 또는 `npm run start`를 재시작하세요.
...
같은 네트워크의 다른 기기에서 접속하려면 `TILEFORGE_WEB_HOST="0.0.0.0"`으로 실행하고, 브라우저에서는 서버 PC의 실제 IP와 포트를 사용합니다.
