...
export function memoryTrafficFor(hw: HardwareConfig, shape: MatmulShape, tile: TileCandidateResult): MemoryTrafficBreakdown {
  ...
  const estimatedSram = tile.predictedSramAccessBytes;
  const estimatedDram = tile.predictedDramAccessBytes;
  return {
    opName: shape.opName,
    aBytes, bBytes, cReadBytes, cWriteBytes,
    dramReadBytes: Math.round(estimatedDram ?? (aBytes+bBytes+cReadBytes)*dataflowMultiplier),
    dramWriteBytes: cWriteBytes,
    sramReadBytes: Math.round(estimatedSram ?? (aBytes+bBytes+cReadBytes+cWriteBytes)*Math.max(1, kt)*dataflowMultiplier),
    sramWriteBytes: Math.round((aBytes+bBytes+cWriteBytes)*0.5),
    reuseNote: hw.dataflow === "WS" ? "weight가 M tile 전반에서 재사용되는 것으로 모델링했습니다" : hw.dataflow === "OS" ? "partial sum이 더 local하게 유지되는 것으로 모델링했습니다" : "M/K tile 반복에서 input이 재사용되는 것으로 모델링했습니다"
  };
}
