...
export function createZip(files: Record<string, string | Buffer>): Buffer {
  ...
  for (const [rawName, content] of Object.entries(files)) {
    const name = rawName.replace(/^\/+/, "").replace(/\.\./g, "_"); const nameBuf = Buffer.from(name); const data = Buffer.isBuffer(content) ? content : Buffer.from(content, "utf8"); const crc = crc32(data);
