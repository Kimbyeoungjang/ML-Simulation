...
function shapeKey(req: SearchRequest, shape: MatmulShape) {
  return hashObject({ m: shape.m, n: shape.n, k: shape.k, dtypeBytes: shape.dtypeBytes, hardware: req.hardware, candidates: req.candidates, objective: req.objective, calibration: req.calibration, scaleSim: req.scaleSim });
}
...
export async function estimateWithClusterPool(req: SearchRequest): Promise<SearchResponse & { computePoolStats?: ComputePoolStats }> {
  ...
  if (shouldUseThreads) {
    try {
      ...
      for (const { key, shape } of representatives) clusterResults.set(key, estimateForShape(req.hardware, shape, req.candidates, req.objective, req.maxResultsPerOp ?? 32, req.calibration, req.scaleSim));
