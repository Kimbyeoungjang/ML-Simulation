...
function formatKiBFromBytes(value: number | undefined): string {
  if (value === undefined || !Number.isFinite(value)) return "-";
  return `${(value / 1024).toFixed(1)} KiB`;
}

function formatKiBFromElements(value: number | undefined, bytesPerElement: number): string {
  if (value === undefined || !Number.isFinite(value)) return "-";
  return `${((value * bytesPerElement) / 1024).toFixed(1)} KiB`;
}

function formatTrafficComparison(predictedBytes: number | undefined, actualElements: number | undefined, bytesPerElement: number): string {
  const predicted = formatKiBFromBytes(predictedBytes);
  const actual = formatKiBFromElements(actualElements, bytesPerElement);
...
function externalComparisonMarkdown(
  ...
  const bytesPerElement = Math.max(1, res.request.hardware.bytesPerElement ?? 2);
  const top3Ratio = scale?.tileValidationCycleRatio;
  ...
  if (scale?.candidateLayers?.length) {
    ...
    lines.push(
      "",
      "- `SCALE-Sim layout cycle`은 top3 topology의 해당 row 1회 실행 cycle입니다.",
      "- `SCALE-Sim total cycle`은 `layout cycle × tileCount`로 환산한 값이므로 TileForge tiled total cycle과 같은 기준입니다.",
      "- SRAM/DRAM 실제값은 SCALE-Sim access report의 element count에 하드웨어 `원소당 byte`를 곱해 KiB로 표시했습니다.",
    );
  }
  lines.push("");
  return lines.join("\n");
...
function externalAppliedQuickMarkdown(
  ...
  const fullTopologyRatio =
    scale?.cycleRatio !== undefined ? scale.cycleRatio.toFixed(3) : "해당 없음";
  const tileRatio =
    scale?.tileValidationCycleRatio !== undefined
      ? scale.tileValidationCycleRatio.toFixed(3)
      : "해당 없음";
  const verdict =
  ...
  return [
    ...
    `  - 근거: ${scaleApplied ? `COMPUTE_REPORT.csv 파싱 완료. full-topology sum cycle ${scale?.totalCycles?.toLocaleString()}, 참고 ratio ${fullTopologyRatio}배, top3 tile 검증 ratio ${tileRatio}배` : (scale?.error ?? "실행 결과 없음")}`,
...
function externalReportMarkdown(
  ...
  const fullTopologyRatio =
    scale?.cycleRatio !== undefined
      ...
      : "해당 없음";
  const tileValidationRatio =
    scale?.tileValidationCycleRatio !== undefined
      ? `${scale.tileValidationCycleRatio.toFixed(4)}배`
      : "해당 없음";
  ...
  return (
    [
      ...
      `- 보고서에서 확인할 위치: report.md의 "2-1. 실제 외부 도구 반영 상태"와 "2-2. 예측 결과와 실제 실행 결과 비교"`,
      `- 원본 산출물: ${scale?.computeReport ?? "COMPUTE_REPORT 없음"}, ${iree?.vmfb ?? "VMFB 없음"}`,
      "",
      "## 1. TileForge estimator 기준값",
      `- 전체 tiled estimate cycle: ${res.summary.totalCycles.toLocaleString()}`,
      `- 전체 예상 시간: ${res.summary.totalTimeUs.toFixed(3)} us`,
      "",
      "## 2. SCALE-Sim 실제 실행 결과",
      `- 상태: ${scale?.ok ? "성공" : scale ? "실패" : "실행 안 함"}`,
      `- 사용 명령: ${scale?.command ?? "해당 없음"}`,
      `- full-topology COMPUTE_REPORT: ${scale?.computeReport ?? "해당 없음"}`,
      `- full-topology row 수: ${scale?.layerCount ?? "해당 없음"}`,
      `- full-topology row cycle 합: ${scale?.totalCycles?.toLocaleString() ?? "해당 없음"}`,
      `- full-topology / TileForge tiled 참고 ratio: ${fullTopologyRatio}`,
      `- top3 tile validation ratio(SCALE-Sim extrapolated / TileForge): ${tileValidationRatio}`,
      scale?.candidateMapePct !== undefined ? `- top3 tile MAPE: ${scale.candidateMapePct.toFixed(2)}%` : "",
      ...
      "",
      "## 4. 기준별 해석",
      "- full-topology cycle은 SCALE-Sim의 `topology.csv` row cycle 합입니다. 이 값은 TileForge가 내부에서 계산하는 tiled total cycle과 분모가 다를 수 있으므로 정확도 지표로 쓰지 않습니다.",
      "- estimator 정확도는 top3 tile validation ratio와 MAPE를 기준으로 봅니다. 이 값은 SCALE-Sim layout 1회 cycle에 tileCount를 곱해 TileForge tiled estimate와 같은 기준으로 환산합니다.",
      "- SRAM/DRAM traffic 비교는 SCALE-Sim access report의 element count에 하드웨어 원소당 byte를 곱해 KiB로 맞춘 뒤 표시합니다. SRAM 작업 영역(capacity footprint)과 access traffic은 별도 지표입니다.",
      "- IREE compile 성공은 실행 성능 측정이 아니라 컴파일 가능성 검증입니다. 실제 런타임 성능은 별도 benchmark가 필요합니다.",
    ]
      .filter(Boolean)
      .join("\n") + "\n"
  );
