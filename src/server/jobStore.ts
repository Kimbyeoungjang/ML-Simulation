...
import { assertSafeJobId, ensureJobRoot, getJobRoot, jobDir } from "./workspace";
import { nowIso, stableId } from "@/lib/determinism";
import { deleteJobSqlite, listJobsSqlite, listJobsSqlitePaged, markStageSqlite, mirrorLog, readJobSqlite, saveJobSqlite, sqlitePrimary } from "./sqliteStore";
import { atomicWriteFile } from "./atomic";
import { appendJobEvent, type StructuredJobEvent } from "./eventsLog";
...
const DEFAULT_TIMEOUT_MS = intEnv("TILEFORGE_JOB_TIMEOUT_MS", 10 * 60 * 1000, 1_000, 24 * 60 * 60 * 1000);
function emitJobEventSafe(jobId: string, event: Omit<StructuredJobEvent, "time" | "jobId">) {
  appendJobEvent(jobId, event).catch((error) => {
    console.warn(`[tileforge] failed to append job event for ${jobId}:`, error);
  });
}

const DEFAULT_MAX_ATTEMPTS = intEnv("TILEFORGE_JOB_MAX_ATTEMPTS", 1, 1, 20);
function maxJobLogLines(): number { return intEnv("TILEFORGE_MAX_JOB_LOG_LINES", 2_000, 100, 100_000); }
...
export async function updateJobStatus(job: JobRecord, status: JobStatus, log?: string) {
  ...
  emitJobEventSafe(job.id, { level: status === "failed" ? "error" : "info", stage: job.stage, code: `STATUS_${status.toUpperCase()}`, message: log ?? `status=${status}` });
...
function appendLogSync(job: JobRecord, log: string) {
  ...
  const line = `[${at}] ${log}`;
  job.logs.push(line);
  const limit = maxJobLogLines();
  if (job.logs.length > limit) job.logs = job.logs.slice(-limit);
  try { mirrorLog(job.id, line, at); }
  catch (error) { console.warn(`[tileforge] sqlite log mirror failed for job ${job.id}:`, error); }
  emitJobEventSafe(job.id, { level: log.startsWith("WARNING") ? "warn" : "info", stage: job.stage, message: log });
}
...
export async function updateProgress(job: JobRecord, stage: JobStage, progress: number, log?: string) {
  ...
  try { markStageSqlite(job.id, stage, progress >= 100 ? "done" : "running", at, log); }
  catch (error) { console.warn(`[tileforge] sqlite stage mirror failed for job ${job.id}:`, error); }
  emitJobEventSafe(job.id, { level: "info", stage, code: "STAGE_PROGRESS", message: log ?? `progress=${progress}`, data: { progress } });
  if (log) appendLogSync(job, log);
  await saveJob(job);
}
...
export async function markStageDone(job: JobRecord, stage: JobStage, detail?: string) {
  ...
  try { markStageSqlite(job.id, stage, "done", at, detail); }
  catch (error) { console.warn(`[tileforge] sqlite stage mirror failed for job ${job.id}:`, error); }
  await saveJob(job);
}
...
export async function listJobsPaged(options: ListJobsOptions = {}): Promise<{ jobs: JobRecord[]; nextCursor?: string; total: number }> {
  ...
  if (sqlitePrimary()) {
    try {
      const sqlitePage = listJobsSqlitePaged({ limit, cursor: options.cursor, status: options.status, since: options.since });
      if (sqlitePage) {
        return {
          jobs: sqlitePage.jobs.map(sanitizeJobForDisplay),
          nextCursor: sqlitePage.nextCursor,
          total: sqlitePage.total
        };
      }
    } catch (error) {
      console.warn("[tileforge] sqlite paged job listing failed; falling back to file scan:", error);
    }
  }
