...
export function EstimatorDatasetPanel({
  ...
  return (
    <section className="suite-section" title="여러 CSV 파일을 업로드해 하나의 대형 학습 dataset으로 병합하고 바로 학습합니다.">
      ...
      <div className="row2">
        <label className="mini-field"><span>CSV 파일 업로드</span><input type="file" accept=".csv,text/csv" multiple onChange={(e) => void onDatasetFilesSelected(e)} /></label>
      ...
      <div className="estimator-actions">
        <button onClick={() => onImportDataset(datasetFiles, datasetTrainAfterImport)} disabled={busy || datasetFiles.length === 0}>{datasetTrainAfterImport ? "CSV 병합 후 자동 학습" : "CSV 병합/검증만 실행"}</button>
