...
import { JobArtifactText, jobById, jobDisplayName } from "./jobArtifacts";
import { JobExternalLogs, fmtBytes } from "./externalLogPanel";
    ...
    <>
      ...
      >
        ...
        <span className="small">
          {fallback
            ...
            : (() => {
                const j = jobById(jobsPayload, sourceJobId);
                return j
                  ? `${jobDisplayName(j)}의 report.md를 보고 있습니다.`
                  : `선택한 작업의 report.md를 보고 있습니다.`;
              })()}
        </span>
      </div>
      <section
        className="report-picker"
        title="완료된 작업별 report.md를 골라 봅니다."
      >
        <div>
          <b>작업별 보고서 선택</b>
          <p className="small">
            완료/실패한 작업 중 report.md artifact가 있는 작업을 선택하면 보고서
            탭이 해당 작업 결과로 바뀝니다.
          </p>
        </div>
        <div className="report-picker-controls">
          <select
            value={sourceJobId}
            onChange={(e) =>
              e.target.value && onSelectJobReport(e.target.value)
            }
          ...
          {sourceJobId && (
            <button
              className="secondary"
              onClick={() => onDeleteJob(sourceJobId)}
              title="현재 보고 있는 작업과 artifact를 삭제합니다."
            >
              현재 작업 삭제
            </button>
          )}
      ...
      {sourceJobId ? (
        <>
          <JobArtifactText
            jobId={sourceJobId}
            path="purpose_gate.md"
            title="선택 작업의 purpose_gate.md"
          />
          <JobArtifactText
            jobId={sourceJobId}
            path="confidence.md"
            title="선택 작업의 confidence.md"
          />
        </>
      ) : (
        <Artifact
...
export function ExternalStatusOverview({ report }: { report: string }) {
  ...
  return (
    <section
      className="external-status-cards"
      title="report.md의 2-1 섹션을 표 대신 카드로 정리한 요약입니다."
    >
      <div className="external-status-heading">
        <div>
          <b>실제 외부 도구 반영 상태</b>
          <p className="small">
            보고서의 2-1 섹션을 읽기 쉽게 카드로 재구성했습니다.
          </p>
        </div>
        {verdict ? (
          <span
            className={`badge ${verdict === "성공" ? "ok-badge" : verdict.includes("대기") ? "warn-badge" : "err-badge"}`}
          >
            최종 판정: {verdict}
          </span>
        ) : null}
      ...
      <div className="external-status-grid">
        {items.map((item) => (
          <article
            key={item.label}
            className={`external-status-card ${statusClass(item.status)}`}
          >
...
export function parseExternalStatus(
  report: string,
): Array<{ label: string; status: string; reason: string }> {
  const section =
    report.match(
      /## 2-1\. 실제 외부 도구 반영 상태\n([\s\S]*?)(?=\n## 2-2\.|\n## 3\.|$)/,
    )?.[1] ?? "";
  ...
  const rows = section
    ...
    .filter(
      (line) =>
        line.startsWith("|") && !line.includes("---") && !line.includes("항목"),
    );
  return rows
    .map((row) => {
      const cols = row
...
export function ResourceMonitor({
  ...
  const [parallelDraft, setParallelDraft] = useState(
    String(capacity?.parallelLimit ?? 2),
  );
  useEffect(() => {
    if (capacity?.parallelLimit != null)
      setParallelDraft(String(capacity.parallelLimit));
  }, [capacity?.parallelLimit]);
  return (
    <section
      className="resource-panel"
      title="서버 프로세스가 보는 CPU/RAM 사용량과 job 병렬 실행 여유를 표시합니다."
    >
      <div className="resource-header">
        <h3>서버 리소스</h3>
        <span className="small">
          {payload?.createdAt
            ? `갱신 ${new Date(payload.createdAt).toLocaleTimeString()}`
            : "상태 수집 중"}
        </span>
      </div>
      <div className="resource-cards">
        <div className="resource-card">
          ...
          <strong>
            {cpu?.sampleBased
              ? `${cpu.overallPct?.toFixed?.(1) ?? cpu.overallPct}%`
              : "측정 준비 중"}
          </strong>
          <div className="bar">
            <span
              style={{
                width: `${Math.min(100, Number(cpu?.overallPct ?? 0))}%`,
              }}
            />
          </div>
          <p className="small">
            코어 {cores.length || payload?.cpuCount || "-"}개
          </p>
        </div>
        <div className="resource-card">
          <b>RAM</b>
          ...
          <div className="bar">
            <span
              style={{ width: `${Math.min(100, Number(mem?.usedPct ?? 0))}%` }}
            />
          </div>
          <p className="small">
        ...
        <div className="resource-card">
          ...
          <strong>
            {capacity
              ? `${capacity.availableSlots}/${capacity.parallelLimit}`
              : "-"}
          </strong>
          <p className="small">
            running {capacity?.runningJobs ?? 0}, queued{" "}
            {capacity?.queuedJobs ?? 0}
          </p>
          <p className="small">
            {capacity?.note ?? "작업 상태를 불러오는 중입니다."}
          </p>
        </div>
      </div>
      <div
        className="parallel-config"
        title=".env의 TILEFORGE_MAX_PARALLEL_JOBS 값을 바꾸고 현재 서버 프로세스에도 즉시 반영합니다."
      >
        <div>
          <b>병렬 실행 수</b>
          <p className="small">
            큐에 들어간 작업을 동시에 몇 개까지 실행할지 설정합니다. 저장하면
            .env에 반영됩니다.
          </p>
        </div>
        <div className="parallel-controls">
          ...
          />
          <button onClick={() => onParallelChange(Number(parallelDraft))}>
            .env에 저장
          </button>
        </div>
      </div>
      {cores.length > 0 && (
        <div
          ...
          title="코어별 CPU 사용률입니다. 첫 조회 직후에는 0으로 보일 수 있고, 다음 자동 갱신부터 실제 변화량 기반 값이 표시됩니다."
        >
          {cores.map((core: any) => (
            <div key={core.index} className="cpu-core">
              ...
              <div className="mini-bar">
                <span
                  style={{
                    width: `${Math.min(100, Number(core.usagePct ?? 0))}%`,
                  }}
                />
              </div>
            </div>
